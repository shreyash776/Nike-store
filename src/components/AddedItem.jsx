...
const AddedItem = (props) => {
  const {cart,setCart}=useContext(CartContext);
    const {title,color,shadow,price,img,id}=props;
    ...
    const incrementQuantity = () => {
        ...
        const decrementQuantity = () => {
          ...
          const deleteItem = (id) => {
            setCart((prevCart) => prevCart.filter((item, index) => item.customKey!== (id)));
    ...
    <>
        <div className='added-item-box' style={{ marginTop:'10px',width:'90%',height:'6rem', display:'flex',justifyContent:'space-around',alignItems:'center'}}>
          <div style={{position:'relative',background:color,boxShadow:shadow,width:'130px',height:'80px',borderRadius:'4px'}}>
            <div style={{position:'absolute',backgroundRepeat:'no-repeat',backgroundImage:`url(${img})`,backgroundSize:'contain',width:'120px',height:'70px',top:'0',right:'3px'}}>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M5 12h14" />
          ...
          <div className='delete' style={{height:'70%',width:'25px',display:'flex',flexDirection:'column-reverse',backgroundColor:'blue'}}> 
          <svg onClick={()=>deleteItem(id)}
